...
> vault 里已经有 **9 部书、840 条**切好的原文；唐宋层七部自动构建底本及一部待核残卷已经入库。
...
│   │       ├── 六壬断案/           222 条（南宋占例）
│   │       ├── 壬归/               43 条（题宋，存疑）
│   │       └── 卜筮书残卷/           2 条（唐抄系现代整理本，原卷待核）
│   ├── 15-导读/                ★ 导读卡（读原文前的七件事，按节生成）
...
│   ├── guide.py                ★ 导读卡生成器（骨架＋可计算的五件，内容由 agent 填）
│   ├── signal_log.py           ★ 对话摩擦打点器（把「这节课哪里不顺」变成数据）
│   ├── retro.py                ★ 教学复盘引擎（跑规则 → 起草提案 → 等你批准）
│   ├── retro_selftest.py       复盘规则边界自测（47 项，含导读 R12–R14）
...
| **一节课怎么开场读书** | `00-索引/导读流程-方案.md` ＋ `00-索引/导读进度.md` | 读原文前先过导读七件套：定位、文本体检、切块读法、生词护栏、待答问题、冲突预警、时间预算。导读只交代读法，不讲原理 |
...
1. Obsidian 打开 `00-索引/阅读路线图.md`，按阶段读底本笔记，读到卡壳就在 Trae 里问。
   **读原文之前它必须先给导读**（见「一·五」），导读讲完会问你两句回执。
...
以前它开场就直接开讲，读哪一段、哪些先不管、这条接在上一课哪里，全靠临场决定。现在
固定成一套导读，**读原文之前**必须过一遍：

```bash
python3 tools/guide.py --entry 涉害法 --check    # 有没有卡、占位填没填、有没有过期
python3 tools/guide.py --entry 涉害法            # 缺卡就生成骨架，内容由 agent 填
python3 tools/guide.py --grades                  # 三档的时间与字数上限
python3 tools/guide.py --list                    # 已有导读卡一览
python3 tools/guide.py --list --stale            # 只列被底本改动甩过期的
```

**七件套**：① 坐标（册篇节页、课序、接在上一课哪个产出上）② 文本体检（净字数、有无
今注与盘、是否今人补入、字词风险）③ 切块与读法（逐块标性质与精读／略读／跳过，跳过
要写回收时点）④ 生词护栏（未通关的前置只当名词）⑤ 待答问题（只摆问题不给答案）
⑥ 冲突预警（同名条目与唐宋层线索，只挂号）⑦ 阅读预算。

**档位按正文净字数判，不按类型**——实测主线 257 条里，毕法有 14 条不足 200 字，课体有
67 条落在 200–3000 字，按类型判会给短口诀派长导读：

| 档 | 净字数 | 导读上限 | 主线条数 |
| :---: | :--- | :--- | ---: |
| A | < 200 字 | ≤ 5 分钟 / 400 字 | 26 |
| B | 200–3000 字 | ≤ 10 分钟 / 700 字 | 220 |
| C | > 3000 字 | ≤ 15 分钟 / 1000 字，必须给分区表 | 11 |

**导读不是提前讲一遍**。它只交代读法，原理归第一拍「解释」；卡里还留着
`TODO(agent)` 就是骨架，不许拿来开讲，`selfcheck.py` 会把它报出来。

**回执是软门槛**：进四拍前它问你两句——这一条要解决什么问题？本节先读哪块、先不读
哪块？答不出不阻断这节课，只补做第 1、3 两件，但会落一条 `fail`：

```bash
python3 tools/guide.py --entry 涉害法 --receipt pass --minutes 4
```

`fail` 和超时由程序自动打点（`导读不足`／`导读超时`），攒够阈值收尾复盘会起草提案：
单节两次导读不足 → 改该档模板（R12）；连续两节 fail → 降档或拆节（R13）；同档连续三节
超时 → 压件数或调预算（R14）。导读卡**按节生成，不批量预生成**：257 条一次性生成只会
堆出一堆空壳卡，且护栏与冲突预警会随进度过期。进度看 `00-索引/导读进度.md`。
...
```bash
python3 tools/signal_log.py types                    # 看十一种信号分别指什么
...
十一种信号：`卡顿` `纠错` `重复追问` `口径分歧` `操作摩擦` `前置缺失` `图缺失` `跳步`
`超时` `导读不足` `导读超时`。最后两种由 `guide.py --receipt` 自动打，不用手敲。
...
```bash
python3 tools/retro.py --rules      # 十三条规则和全部阈值（含导读 R12–R14）
...
```bash
python3 tools/retro_selftest.py     # 47 项边界用例，规则改了必须同时改用例
...
2. 顶层执行：
   ...
   - **`sources/` 的原书不入库**（体积大）。所以换台机器 clone 下来 `sources/` 是空的，
     `./一键部署.sh` 会跳过所有书 —— 但 `六壬vault/` 里已有的 840 条条目照常同步，
